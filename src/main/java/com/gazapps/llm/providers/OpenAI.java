...
/**
 * Implementação da interface Llm para OpenAI.
 * Usa o mesmo formato que Groq, pois ambos seguem a API padrão OpenAI.
 */
public class OpenAI implements Llm {
    ...
    public OpenAI() {
        ...
        // Definir capacidades do OpenAI
        this.capabilities = LlmCapabilities.builder()
            .functionCalling(true)
            .systemMessages(true)
            .streaming(true)
            .maxTokens(128000)
            .supportedFormats(java.util.Set.of("text", "json", "image"))
            .build();
        
        config.isLlmConfigValid(LlmProvider.OPENAI);
        
        if (debug) {
            logger.debug("🔧 OpenAI initialized with capabilities: {}", capabilities);
        }
    }
    ...
    public LlmResponse generateWithTools(String prompt, List<ToolDefinition> tools) {
        try {
            ...
            if (conversationLogger.isInfoEnabled()) {
                conversationLogger.info("=== OPENAI REQUEST ===");
            ...
            // === LOGGING RESPONSE ===
            if (conversationLogger.isInfoEnabled()) {
                conversationLogger.info("=== OPENAI RESPONSE ===");
                ...
                conversationLogger.info("=== END OPENAI ===");
            }
            ...
            if (conversationLogger.isErrorEnabled()) {
                conversationLogger.error("=== OPENAI ERROR ===");
                conversationLogger.error("Error: {}", e.getMessage());
                conversationLogger.error("=== END OPENAI ERROR ===");
            }
    ...
    public boolean isHealthy() {
        try {
            ...
            logger.warn("Health check failed for OpenAI: {}", e.getMessage());
            return false;
        }
    }
    ...
    private void validateInput(String prompt) {
        ...
        if (prompt.length() > capabilities.getMaxTokens()) {
            throw new LlmException(getProviderName(), LlmException.ErrorType.INVALID_REQUEST, 
                                 "Prompt too long for OpenAI (maximum " + capabilities.getMaxTokens() + " characters)");
        }
    }
    
    private HttpRequest buildRequest(String prompt, List<com.gazapps.llm.tool.ToolDefinition> tools) throws Exception {
        OpenAIRequest requestObject = new OpenAIRequest(prompt, tools, model);
    ...
    private HttpResponse<String> sendRequest(HttpRequest request) throws Exception {
        ...
        if (debug) {
            logger.debug("OpenAI response status: {}", response.statusCode());
            logger.debug("OpenAI response body: {}", response.body());
        }
        
        if (response.statusCode() >= 400) {
            String errorMessage = "OpenAI API error: " + response.body();
    ...
    private LlmResponse parseResponse(HttpResponse<String> response) throws Exception {
        ...
        OpenAIResponse.Message message = choice.message;
        ...
        if (message.toolCalls != null && message.toolCalls.length > 0) {
            ...
            return LlmResponse.error("Invalid response format from OpenAI");
        }
    }
    ...
    // Classes internas para request/response do OpenAI (similar ao Groq)
    ...
    private static class OpenAIRequest {
        ...
        public OpenAIRequest(String prompt, List<com.gazapps.llm.tool.ToolDefinition> toolDefinitions, String model) {
            this.model = model;
            ...
            this.tools = (toolDefinitions != null && !toolDefinitions.isEmpty()) ? 
                         toolDefinitions.stream()
                             .map(tool -> new ToolDefinition(tool.toOpenAiFormat()))
        ...
        private static class ToolDefinition {
            ...
            public ToolDefinition(Map<String, Object> openaiFormat) {
                this.function = new Function((Map<String, Object>) openaiFormat.get("function"));
            }
        }
